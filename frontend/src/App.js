...
function App() {
  ...
  const {userContext, setUserContext, emptyCart} = useContext(UserContext);
  ...
  const verifyUser = useCallback( async() => {
    ...
    try {
        console.log({Message: "Call verifyLogin", DateTime: new Date()});
        let res = await axios.post(process.env.REACT_APP_API_ENDPOINT + "auth/verifyLogin")    
        const data = res.data;
        setUserContext((prev) => {
  ...
  const logoutAction = async () => {
    try {
      await axios.get(process.env.REACT_APP_API_ENDPOINT + "auth/logout", { headers: {"Authorization" : `Bearer ${userContext.token}`} });
      setUserContext( (prev) => {
